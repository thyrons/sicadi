<?php
  ...
  $contador=0;
  conectarse();	  
  $consulta=pg_query("select * from provincia");
  while($row=pg_fetch_row($consulta)){
    $contador=$row[0];
  }
  $contador=$contador+1;
  if(isset($_POST['accion'])=='add'){  
    $consulta=pg_query("select * from provincia where nombre_provincia='$_POST[nomProvincia]'");
    $cont=pg_num_rows($consulta);
    if($cont>0){
      $error=2;
    }
    if($error==0){
      pg_query("insert into provincia values ('$contador','$_POST[nomProvincia]','$_POST[comboPais]')");
      $error=0;
    }
    if($error==0){
        echo "{success: true}";
